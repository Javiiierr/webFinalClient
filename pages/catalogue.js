...
export default function Catalogue() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const cachedData = localStorage.getItem(`page_${currentPage}`);
      if (cachedData) {
        setGameData(JSON.parse(cachedData));
      } else { // if nothing in cache
        const res = await fetch(`https://api.rawg.io/api/games?key=065460f6fbc3465a8a19dafdc359151f&page_size=12&page=${currentPage}`); // call API to get main game info for current page
        ...
        localStorage.setItem(`page_${currentPage}`, JSON.stringify(gamesWithDescription)); // save results and search to local storage
      }
    };
    fetchData();
  }, [currentPage]); // called by useEffect
